...
Wander is not intended for children under 13 and is not designed as a children’s app. Wander does not knowingly collect personal information from children under 13.
...
Wander 不收集你的真实身份信息。Wander 会处理有限的匿名功能数据，用于 App 功能、每日额度、防重复抽取、收藏、完成记录、网络重试幂等和反算法内容多样性。
...
Wander 不面向 13 岁以下儿童，也不是儿童 App。Wander 不会故意收集 13 岁以下儿童的个人信息。
